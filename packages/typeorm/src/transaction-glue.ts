...
// ---------------------------------------------------------------------------
// PRIVATE to @kronos-ts/typeorm. Not exported from the package barrel.
//
...
// uses them, so this package owns its copy, carrying only what typeorm does.
// ---------------------------------------------------------------------------
 ...
 * part in here: this package ships a FINISHED factory (`typeormUnitOfWork(make,
 * manager)`) and a finished accessor pair, because it already knows what a
 ...
 * DATABASE: two `typeormUnitOfWork(make, a)` / `typeormUnitOfWork(make, b)` factories share this
 ...
 * exactly as it went in, so `typeormUnitOfWork(() => Object.assign(unitOfWork(),
 * { probe: true as const }), manager)` still has `probe` on it at runtime, and the WeakMap
...
/**
 * The LAZY-OPENING half of the accessor pair — what `typeormTransaction(uow)` calls.
 *
...
/**
 * The OBSERVING half of the accessor pair — what `activeTypeormTransaction(uow)` calls.
 *

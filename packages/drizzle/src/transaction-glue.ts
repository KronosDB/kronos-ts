...
// ---------------------------------------------------------------------------
// PRIVATE to @kronos-ts/drizzle. Not exported from the package barrel.
//
...
// uses them, so this package owns its copy, carrying only what drizzle does.
// ---------------------------------------------------------------------------
 ...
 * part in here: this package ships a FINISHED factory (`drizzleUnitOfWork(make,
 * db)`) and a finished accessor pair, because it already knows what a
 ...
 * DATABASE: two `drizzleUnitOfWork(make, a)` / `drizzleUnitOfWork(make, b)` factories share this
 ...
 * exactly as it went in, so `drizzleUnitOfWork(() => Object.assign(unitOfWork(),
 * { probe: true as const }), db)` still has `probe` on it at runtime, and the WeakMap
...
/**
 * The LAZY-OPENING half of the accessor pair — what `drizzleTransaction(uow)` calls.
 *
...
/**
 * The OBSERVING half of the accessor pair — what `activeDrizzleTransaction(uow)` calls.
 *

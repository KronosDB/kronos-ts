...
- 4d5f939: `postgresHandler` opens the unit of work's transaction when the handler is its first writer.
...
- e923adc: Removed the `@kronos-ts/postgres/drizzle` and `@kronos-ts/postgres/kysely` subpaths, along with the optional `drizzle-orm` and `kysely` peer dependencies. BREAKING.
  ...
  } from "@kronos-ts/postgres/drizzle";
  const wrap = (h) =>
    postgresHandler(
      drizzleHandler(h, (client: Sql) => drizzle(client)),
      pg
    );
  ```
...
- 5ab718d: `db.statement` spans carry the statement as `db.query.text`.
...
- 446e13b: Statement spans follow the trace, and the builder capabilities name their own types.
...
- 84a74d9: Queries always run in a task of their own, correlation belongs to the invocation, and statements are observable on the postgres family.

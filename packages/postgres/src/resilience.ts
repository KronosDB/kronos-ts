 ...
 *
 * Exponential backoff with full jitter, per-event attempt caps and a
 * terminal-error short-circuit. Nothing here is exported from this package's
 * barrel.
 *
 ...
 *
 * Each consuming package keeps its own copy, carrying only the FUNCTIONS it
 * uses — this one has no health probe, because a pool bootstraps and then
 * either works or throws. The config shape is unchanged, since that is what
 * `postgresPool({ resilience })` callers write.
 */

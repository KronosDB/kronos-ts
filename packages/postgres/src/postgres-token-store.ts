 ...
 * client is the point:
 *
 * PRINCIPLE: every write goes through {@link sharedPostgresTransaction}, so a
...
export function postgresTokenStore(
  ...
  return {
    ...
    async get(processorName, segment, uow) {
      return readToken(await rowFor(await sql(uow), processorName, segment))
    },
    ...
    async fetchSegments(processorName, uow) {
      const rows = await (await sql(uow)).query<{ segment: number | string }>(
    ...
    async deleteToken(processorName, segment, uow) {
      await (await sql(uow)).query(`DELETE FROM ${table} WHERE processor_name = $1 AND segment = $2`, [
        processorName,
        segment,
      ])
    },

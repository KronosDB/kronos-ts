 ...
 * live postgres. The real postgresUnitOfWork is used unchanged — these tests
 * verify the scheduler's SQL + lifecycle, not the tx-bridging glue (covered by
 * postgres-transaction.test.ts).
 */

import { describe, it, expect } from "bun:test"
import { qn, type Metadata } from "@kronos-ts/core"
...
function createFakeAdapter() {
  ...
  function execOn(map: Map<string, FakeRow>, sql: string, params: unknown[] = []): unknown[] {
    ...
    // Per-transaction safety GUCs armed by the concrete adapters via SET
    // LOCAL. No-op for the fake — they only matter against real postgres.
...
function createFakeEventStore() {
  ...
  } as unknown as import("@kronos-ts/core").EventStore
...
function wire(opts: { adapter: PostgresAdapter; store: import("@kronos-ts/core").EventStore }) {
...
describe("postgresEventScheduler", () => {
  describe("schedule()", () => {
    ...
    it("rolling back the UoW after schedule() leaves no row behind", async () => {
      ...
      const event = makeEvent()
      await expect(
        uowFactory().execute(async (uow) => {
          await scheduler.schedule(event, new Date(Date.now() + 60_000), uow)
    ...
    it("throws when handed no UnitOfWork (must join a transaction)", async () => {
      const fake = createFakeAdapter()
      const { store } = createFakeEventStore()
  ...
  describe("cancel()", () => {
    ...
    it("returns 'already-appended' when the row already fired", async () => {
      ...
      const event = makeEvent()
      const token = await uowFactory().execute(async (uow) => {
        return scheduler.schedule(event, new Date(Date.now() - 1000), uow)
      })
  ...
  describe("worker", () => {
    it("fires due pending schedules via eventStore.append and marks them 'appended'", async () => {
      ...
      await uowFactory().execute(async (uow) => {

...
// ---------------------------------------------------------------------------
// PRIVATE to @kronos-ts/kysely. Not exported from the package barrel.
//
...
// uses them, so this package owns its copy, carrying only what kysely does.
// ---------------------------------------------------------------------------
 ...
 * part in here: this package ships a FINISHED factory (`kyselyUnitOfWork(make,
 ...
 * DATABASE: two `kyselyUnitOfWork(make, a)` / `kyselyUnitOfWork(make, b)` factories share this
 ...
 * exactly as it went in, so `kyselyUnitOfWork(() => Object.assign(unitOfWork(),
...
/**
 * The LAZY-OPENING half of the accessor pair — what `kyselyTransaction(uow)` calls.
 *
...
/**
 * The OBSERVING half of the accessor pair — what `activeKyselyTransaction(uow)` calls.
 *

 ...
 *
 * ```ts
 * const uow = kyselyUnitOfWork(db, unitOfWork)
 ...
 */
export function kyselyUnitOfWork(db: KyselyDb, make: () => UnitOfWork): () => UnitOfWork {
  return adapterUnitOfWork(registry, transactionHooks(db), make)
}
...
//   2. A UNIT-OF-WORK WRAPPER — `kyselyUnitOfWork(db, unitOfWork)`, a
...
export interface KyselyCapability {
  /**
...
export interface KyselyContext extends HandlerContext, KyselyCapability {}
/** An event handler's context, plus this extension's capability. */
export interface KyselyEventContext extends EventHandlerContext, KyselyCapability {}
/** A query handler's context, plus this extension's capability. */
export interface KyselyQueryContext extends QueryHandlerContext, KyselyCapability {}

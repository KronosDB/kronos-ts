...
- 303f268: `KnexFamily` is `KnexUnitOfWork`, and `KnexContext` is `KnexCommandContext`. BREAKING for
  anyone who spelled either.

  ```ts
  // before
  type Task = CorrelatingUnitOfWork & KnexFamily;
  commandHandler(Edit, async (m, ctx: KnexContext) => {
    ctx.knex();
  });

  // after
  type Task = CorrelatingUnitOfWork & KnexUnitOfWork;
  commandHandler(Edit, async (m, ctx: KnexCommandContext) => {
    ctx.knex();
  });
  ```

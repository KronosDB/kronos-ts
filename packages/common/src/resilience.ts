/**
 * Connection resilience primitives shared by transport extensions
 * (KronosDB, Axon Server). Phase 9 / D-96..D-100 / D-101.
 *
 * - Exponential backoff with FULL jitter
 *   (delay = random(0, base * mult^attempt), capped at maxDelayMs)
 * - Per-event default attempt caps:
 *     initial-connect: 10
 *     reconnect:       30
 *     per-operation:    5
 *     health-check:     3
 * - Terminal-error short-circuit via opts.isRetryable
 * - Health-check warn-then-continue (5s default threshold per D-100)
 */
 ...
 * per-extension knob path (`kronosDb({ resilience: { ... } })`).
 */

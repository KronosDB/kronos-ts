 ...
 * which is what `app.processors` is, so `kronosDbControlPlane(kdb.platform,
...
/**
 * Wires KronosDB's remote administration onto a backend's platform stream.
 *
 ...
 * ## Ordering
 *
 * `platform.start()` is called LAST, after both registrations. Doing it the
 ...
 * why this factory calls `platform.start()` rather than the backend.
 *
 ...
 * stream — so the backend starts the stream itself if nothing else has. Getting
 ...
 * by the platform connection and flushed on registration.
 *
 * @param platform   The backend's platform stream — `kdb.platform`.

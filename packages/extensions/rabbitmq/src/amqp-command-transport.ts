...
export class AmqpRabbitMqCommandTransport implements RabbitMqCommandTransport {
  ...
  constructor(
    ...
    private readonly connectFn: AmqpConnect = defaultAmqpConnect,
  ) {}
  ...
  private async doConnect(): Promise<void> {
    this.connection = await this.connectFn(this.config.url)
    ...
    const reply = await this.channel.assertQueue(this.config.topology.replyQueue(), {
      durable: false,
      exclusive: true,
      autoDelete: true,
    })
...
async function defaultAmqpConnect(url: string): Promise<ChannelModel> {
  const amqp = await import("amqplib") as AmqplibModule

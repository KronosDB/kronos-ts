...
beforeAll(async () => {
  ...
  adapter = pgAdapter({ connectionString: pg.connectionString } satisfies PgAdapterConfig)
...
describe("pgAdapter — query / queryOne", () => {
...
describe("pgAdapter — LISTEN/NOTIFY", () => {
  it("delivers NOTIFY payload to the listen() callback and unlistens cleanly", async () => {
    ...
    await adapter.query(`NOTIFY kronos_pg_test_chan, 'after_unlisten'`)
    await new Promise((r) => setTimeout(r, 250))
    expect(received).toEqual(["hello"])
  })
})

describe("pgAdapter — lifecycle", () => {
  it("disconnect() is idempotent (second call is a no-op, not an error)", async () => {
    ...
    const a = pgAdapter({ connectionString: pg.connectionString })
    await a.connect()
    await a.disconnect()

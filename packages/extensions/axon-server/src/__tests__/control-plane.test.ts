 ...
 *
 * These run against a fake `PlatformConnection` — no container, no gRPC. What
 * they pin down is the contract the extraction has to preserve:
 *
...
import type { PlatformConnection, InstructionHandler, PlatformInstruction } from "../platform-service.js"
...
describe("axonServerControlPlane — registration ordering", () => {
...
describe("axonServerControlPlane — status reporting", () => {
  it("reports a synthetic single segment for a processor with no per-segment status", async () => {
    ...
    expect(status!.segments).toEqual([
      { segmentId: 0, caughtUp: true, replaying: false, onePartOf: 1, tokenPosition: 17n, errorState: "" },
    ])
  })
  ...
  it("maps per-segment processing status when the processor exposes it", async () => {
    ...
    const [status] = platform.statuses()
    expect(status!.segments).toEqual([
      { segmentId: 0, caughtUp: true, replaying: false, onePartOf: 1, tokenPosition: 5n, errorState: "" },
      { segmentId: 1, caughtUp: false, replaying: true, onePartOf: 1, tokenPosition: 9n, errorState: "boom" },
    ])
  })
  ...
  it("reads LIVE processor state on every report", async () => {
    const platform = fakePlatform()
    let running = false
    const proc: ManagedEventProcessor = { name: "p", get running() { return running } }
    await axonServerControlPlane(platform, [proc])

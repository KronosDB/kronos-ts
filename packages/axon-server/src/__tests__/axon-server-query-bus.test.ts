...
describe("axonServerQueryBus — subscription queries", () => {
  ...
  it("handles inbound subscribe — sends initialResult back over the outbound stream", async () => {
    ...
    expect(
      jsonSerializer.deserialize(response.subscriptionQueryResponse.initialResult.payload),
    ).toBe("hello")
  })
  ...
  it("emitUpdate dispatches a SubscriptionQueryResponse.update to each matching tracked subscriber", async () => {
    ...
    expect(jsonSerializer.deserialize(updates[0].subscriptionQueryResponse.update.payload)).toBe(
      "value-v1",
    )
  })

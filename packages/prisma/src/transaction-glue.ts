...
// ---------------------------------------------------------------------------
// PRIVATE to @kronos-ts/prisma. Not exported from the package barrel.
//
...
// uses them, so this package owns its copy, carrying only what prisma does.
// ---------------------------------------------------------------------------
 ...
 * part in here: this package ships a FINISHED factory (`prismaUnitOfWork(make,
 * prisma)`) and a finished accessor pair, because it already knows what a
 ...
 * DATABASE: two `prismaUnitOfWork(make, a)` / `prismaUnitOfWork(make, b)` factories share this
...
/**
 * The LAZY-OPENING half of the accessor pair — what `prismaTransaction(uow)` calls.
 *
...
/**
 * The OBSERVING half of the accessor pair — what `activePrismaTransaction(uow)` calls.
 *

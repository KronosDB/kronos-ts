...
function fakeConnection(captured: { outbound: any[]; outboundIter?: AsyncIterable<any> }, inbound: AsyncIterable<any>): KronosDbConnection {
  return {
    channel: {} as any,
    platform: {} as any,
    commands: {} as any,
    ...
    queries: {
      openStream(outboundIter: AsyncIterable<any>) {
        ...
        // Drain the outbound stream into captured array
        ;(async () => {
          try {
...
describe("kronosDbQueryBus — subscription queries", () => {
  ...
  it("emitUpdate dispatches a SubscriptionQueryResponse.update to each matching tracked subscriber", async () => {
    ...
    await flush(30)
    // Clear initial-result outbounds.
...
// Silence the unused z import in case zod gets stripped — keep here in case
// future tests need it for typed query descriptors.
void z

...
- b853dd5: The event processor no longer pays a millisecond between batches, and no longer runs a no-op claim update per batch.
...
- 926b661: `ctx.send` resolves to the command descriptor's `result` type, as `ctx.query` and the edge verb `send(bus, …)` already do. It was `unknown`. A command with no `result` schema still resolves to `unknown`. Remove casts on `await ctx.send(...)`.

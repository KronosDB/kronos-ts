...
import { qn, emptyMetadata, type QueryMessage } from "../../messaging/messages.js"
...
describe("LocalQueryBus subscription queries", () => {
  ...
  it("defers updates to AFTER_COMMIT when ProcessingContext is provided", async () => {
    ...
    // `runAfterCommitOrImmediately` defers on the unit of work it is HANDED,
    // so the emit must carry one for the update to be deferred at all.
    await unitOfWork().execute(async (uow) => {
      bus.emitUpdate("test.GetCourse", () => true, { deferred: true }, uow)
      log.push("emitted")
      uow.onCommit(() => {
        log.push("committed")
      })

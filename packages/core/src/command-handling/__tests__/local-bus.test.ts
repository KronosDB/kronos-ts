...
import { qn, emptyMetadata, metadataAnd, type CommandMessage } from "../../messaging/messages.js"
import { generateIdentifier } from "../../messaging/identifier.js"
import { localCommandBus } from "../local-bus.js"
import { unitOfWork } from "../../unit-of-work/unit-of-work.js"
import { interceptingCommandBus, type Intercept } from "../../interception/intercepting-bus.js"
...
describe("LocalCommandBus", () => {
  describe("dispatching commands to handlers", () => {
    it("dispatches a command to the registered handler", async () => {
      const bus = localCommandBus(unitOfWork)
...
//
// What used to be `registerDispatchInterceptor`, and then a variadic list of
// transforms, is now ONE function. Plurality composes in function space, where the
...
describe("interceptingCommandBus", () => {
  it("applies the intercept before the command reaches the handler", async () => {
    const inner = localCommandBus(unitOfWork)
  ...
  it("passes subscribe through to the delegate untouched", async () => {
    const inner = localCommandBus(unitOfWork)
    const bus = interceptingCommandBus(inner, correlation)
    let handled = false
  ...
  it("wrapping twice is a no-op on values, not a duplicate stamp", async () => {
    ...
    const bus = interceptingCommandBus(interceptingCommandBus(inner, correlation), correlation)

    const message = commandMsg("Cmd")
    await bus.dispatch(message)

    // `correlation` starts the chain at the message and names it as its own cause;
  ...
  it("correlation preserves an existing correlationId and re-points causation at this message", async () => {
    const inner = localCommandBus(unitOfWork)
    let received: CommandMessage | undefined
    inner.subscribe("test.Cmd", async (msg) => {
      received = msg

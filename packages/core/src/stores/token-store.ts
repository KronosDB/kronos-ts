 ...
 * double-processing.
 *
 * Every method takes the unit of work as a TRAILING parameter. Persistent
 * implementations read the adapter transaction off it
 * (`activeDrizzleTransaction(uow) ?? db`, and the equivalent per adapter) so token updates and projection
 * updates commit atomically. It is optional because the lifecycle calls —
 * `initializeSegments` and the startup `get`, plus `resetTokens` — legitimately
 * run outside any unit of work; those get the plain writer, exactly as the old
 * permissive `getActiveTransaction()` returned `undefined` there.
 */
export interface TokenStore {
   ...
   */
  get(processorName: string, segment: number, uow?: UnitOfWork): Promise<TrackingToken | undefined>
   ...
   */
  initializeSegments(processorName: string, segmentCount: number, uow?: UnitOfWork): Promise<void>
   ...
   */
  claimToken(processorName: string, segment: number, ownerId: string, uow?: UnitOfWork): Promise<TrackingToken | undefined>

  /**
   * Extend the claim lease for a segment without processing.
   * Called periodically to prevent claim expiry during long batches.
   */
  extendClaim(processorName: string, segment: number, ownerId: string, uow?: UnitOfWork): Promise<void>

  /**
   * Release a segment claim, allowing other instances to pick it up.
   */
  releaseClaim(processorName: string, segment: number, ownerId: string, uow?: UnitOfWork): Promise<void>

  /**
   * Fetch all segment IDs that have tokens for a processor.
   * Used during startup to discover available segments.
   */
  fetchSegments(processorName: string, uow?: UnitOfWork): Promise<number[]>

  /**
   * Fetch segment IDs that are not currently claimed by any instance.
   * Used to discover segments available for claiming.
   */
  fetchAvailableSegments(processorName: string, uow?: UnitOfWork): Promise<number[]>

  /**
   * Delete the token for a segment (e.g., after merging).
   */
  deleteToken(processorName: string, segment: number, uow?: UnitOfWork): Promise<void>
}

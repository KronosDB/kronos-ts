...
describe("InMemoryEventStore", () => {
  describe("append and source", () => {
    it("appends events and sources them back by tag query", async () => {
      ...
      const result = await store.source(
        sourcingCondition({ tags: { courseId: "cs-101" } }),
      )
    ...
    it("returns empty events when nothing matches", async () => {
      ...
      // when
      const result = await store.source(
        sourcingCondition({ tags: { courseId: "nonexistent" } }),
      )
    ...
    it("sources events matching either query item", async () => {
      const store = inMemoryEventStore()
      ...
      const result = await store.source(
        sourcingCondition(
          [{ tags: { courseId: "cs-101" } }, { tags: { studentId: "stu-001" } }],
        ),
      )
    ...
    it("sources events with type restriction", async () => {
      ...
      const result = await store.source(
        sourcingCondition(
          { tags: { courseId: "cs-101" }, types: ["test.CourseCreated"] },
        ),
      )
  ...
  describe("append condition", () => {
    ...
    it("succeeds when conflicting events are outside the query", async () => {
      const store = inMemoryEventStore()
      ...
      // then — should succeed because cs-102 event doesn't match our query
      await store.append([event2], condition)
    })
  })
})

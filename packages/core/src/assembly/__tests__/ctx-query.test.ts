...
import { command, commandHandler, event, eventHandler, eventProcessor, inMemoryTokenStore, query, queryHandler, send } from "../../index.js"
import { state } from "../../state/state.js"
import { z } from "zod"
import { kronos } from "../kronos.js"
import { lineage, interceptingCommandBus, interceptingQueryBus, unitOfWork, simpleCommandBus, simpleQueryBus } from "../../index.js"
...
const Order = state({
  ...
  evolve: [[OrderPlaced, (s) => ({ ...s, placed: true })]],
})
...
describe("ctx.query", () => {
  it("lets a command handler consult another module's query handler in-UoW", async () => {
    ...
    const app = kronos({
      states: [{ ...Order, eventStore: orderingLog }],

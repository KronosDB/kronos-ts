...
import { type TagResolver } from "../../stores/tag-resolver.js"
import { command, commandHandler, event, eventHandler, eventProcessor } from "../../index.js"
import { state } from "../../state/state.js"
import { z } from "zod"
import { kronos } from "../kronos.js"
import { lineage, interceptingCommandBus, interceptingQueryBus, inMemoryTokenStore, send, unitOfWork, simpleCommandBus, simpleQueryBus, type UnitOfWork } from "../../index.js"
...
describe("billing", () => {
  it("boots and bills a line against its own event store", async () => {
    ...
    const app = kronos({
      states: slice.states.map((s) => ({ ...s, ...buses, eventStore: persistence })),
  ...
  it("two slices, two event stores, one bus", async () => {
    ...
    const buses = inMemoryBuses()
    const app = kronos({
      states: billingSlice.states.map((s) => ({ ...s, ...buses, eventStore: billingPersistence })),
      commandHandlers: [
  ...
  // snapshotStore / tagResolver) — the command/query buses are strictly
  ...
  it("a handler can override tagResolver too, not just persistence", async () => {
    ...
    const buses = inMemoryBuses()
    const app = kronos({
      states: slice.states.map((s) => ({ ...s, ...buses, eventStore, tagResolver })),
...
describe("component resolution", () => {
  it("a supplied unitOfWork factory reaches the default command bus", async () => {
    ...
    const app = kronos({
      states: slice.states.map((s) => ({ ...s, ...buses, eventStore })),

...
describe("state()", () => {
  ...
  it("an ARRAY of tag records is an OVERRIDE — every record against every folded type", () => {
    ...
    const Subscription = state({
      ...
      evolve: [[CourseCreated, (state) => state]],
    })
...
describe("granular query derivation", () => {
  ...
  it("boot error names the state AND the event type when they share no tag key", () => {
    expect(() =>
      state({
        name: "Course",
        id: { courseId: z.string() },
    ...
    ).toThrow(/"Course".*university\.StudentEnrolledInFaculty.*share no tag key/s)
  })

...
import { subscribeCommandHandlers, type CommandInvocationDeps } from "../command-handling-module.js"
import { commandHandler } from "../command-handler.js"
import { eventHandler } from "../event-handler.js"
import { command, event, type CommandDescriptor } from "../../messages/descriptor.js"
import type { CommandBus } from "../../buses/command-bus.js"
import type { CommandMessage, SequencedEventMessage } from "../../messages/message.js"
import { unitOfWork, type UnitOfWork } from "../../unit-of-work/unit-of-work.js"
import { handlerContext, eventHandlerContext, type HandlerContext } from "../handler-context.js"
...
function deps(overrides: {
  ...
  return {
    ...
    ...(overrides.stateManager ? { stateManager: overrides.stateManager } : {}),
  }
}
  ...
  it("ctx.load resolves state through the configured state manager", async () => {
    ...
    const stateManager = {
      load: async (_module: any, id: any) => {
        loaded.push(id)
        return { state: { exists: true }, sourcingInfo: { query: { tags: {} }, markerPosition: 0n } }
      },
    }
    let result: unknown
    const handler = commandHandler(CreateCourse, async ({ payload }, ctx) => {
    ...
    subscribeCommandHandlers([handler], {
      commandBus: bus,
      ...deps({ commandBus: bus, stateManager }),
    })

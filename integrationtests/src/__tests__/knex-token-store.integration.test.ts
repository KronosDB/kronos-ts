...
describe("Knex TokenStore (PostgreSQL)", () => {
  ...
  beforeAll(async () => {
    ...
    knex = Knex({
      client: "pg",
      connection: `postgresql://test:test@${host}:${port}/test`,
    })
    await knex.raw(TOKEN_TABLE_DDL)
  }, 120_000)
  ...
  beforeEach(async () => {
    await knex.raw("DELETE FROM kronos_token_entries")
  })

  // -- Happy paths --

  it("store and get a token", async () => {
    const store = knexTokenStore(knex as any)
  ...
  it("claimToken succeeds after claim expires", async () => {
    const store = knexTokenStore(knex as any, { claimTimeoutMs: 100 })
    await store.initializeSegments("test-processor", 1)
    await store.claimToken("test-processor", 0, "owner-1")

...
export const createCourse = commandHandler(CreateCourse, async (cmd, _metadata) => {
...
export const changeCourseCapacity = commandHandler(ChangeCourseCapacity, async (cmd, _metadata) => {
  const course = await load(CourseEntity, { courseId: cmd.courseId })
...
export const subscribeStudent = commandHandler(SubscribeStudent, async (cmd, _metadata) => {
  const course = await load(CourseEntity, { courseId: cmd.courseId })
  if (!course.created) throw new Error("Course does not exist")
...
export const unsubscribeStudent = commandHandler(UnsubscribeStudent, async (cmd, _metadata) => {
  const course = await load(CourseEntity, { courseId: cmd.courseId })
  if (!course.created) throw new Error("Course does not exist")
